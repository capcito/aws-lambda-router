...
function process(proxyIntegrationConfig, event, context) {
    ...
    try {
        ...
        errorHandlers.push(actionConfig.onError);
        ...
        return processActionAndReturn(actionConfig, event, context, headers, errorMapping, errorHandlers);
    } catch (error) {
        console.log('Error while evaluating matching action handler', error);
        const result = tryInvokeCustomErrorHandlers(errorHandlers, event, context, error);
        if (result) {
