...
function processActionAndReturn(actionConfig, event, context, headers, errorMapping, errorHandlers) {
    return Promise.resolve(actionConfig.action(event, context)).then(res => {
        ...
        const result = tryInvokeCustomErrorHandlers(errorHandlers, event, context, err);        
        return result || convertError(err, errorMapping, headers);
    });
}
...
function process(proxyIntegrationConfig, event, context) {
    ...
    const errorHandlers = [proxyIntegrationConfig.onError];
    ...
    try {
        ...
        if (proxyIntegrationConfig.onRequest) {
                proxyIntegrationConfig.onRequest(event, context);
        }
            
        return processActionAndReturn(actionConfig, event, context, headers, errorMapping, [actionConfig.onError, proxyIntegrationConfig.onError]);
    } catch (error) {
        console.log('Error while evaluating matching action handler', error);
        const result = tryInvokeCustomErrorHandlers([proxyIntegrationConfig.onError], event, context, error);
        if (result) {
...
function tryInvokeCustomErrorHandlers(errorHandlers, event, context, err) {
    for (let index = 0; index < errorHandlers.length; index++) {
        const handler = errorHandlers[index];
        if (handler) {
            const errorResult = handler(err, event, context)
...
function convertError(error, errorMapping, headers) {
    ...
    try {
        return {
            statusCode: isHttpError ? error.status : 500,
            body: JSON.stringify({
                error: isHttpError ? 'HttpError' : error.name,
                name: error.name,
