...
function processActionAndReturn(actionConfig, event, context, headers, errorMapping) {
    return Promise.resolve(actionConfig.action(event, context)).then(res => {
        ...
        if (actionConfig.onError) {
            return actionConfig.onError(event, context, err);
        }
...
function convertError(error, errorMapping, headers) {
    if (error.status && error.body) {
    ...
    } else if (typeof error.reason === 'string' && errorMapping && errorMapping[error.reason]) {
        return {
        ...
        return {
            statusCode: error.status,
            body: JSON.stringify({ error: "ServerError", message: error.message, stack: error.stack, details:`Generic error:${JSON.stringify(error)}`}),
